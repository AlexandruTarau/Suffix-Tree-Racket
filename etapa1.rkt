...
(define (tail-longest-common-prefix w1 w2 rez)
  (cond
    ((or (st-empty? w1) (st-empty? w2)) (list rez w1 w2))
    ((equal? (car w1) (car w2))
...
(define (longest-common-prefix-of-list words)
  (if (= 1 (length words))
      ...
      (longest-common-prefix-of-list
       (append (cddr words) (list (car (longest-common-prefix (car words) (cadr words)))))
       )
  ))
...
(define (match-pattern-with-label st pattern)
  ...
  (if (list? branch)
     (cond
       ((equal? (car (longest-common-prefix (get-branch-label branch) pattern)) pattern) #t)
       ((equal? (car (longest-common-prefix (get-branch-label branch) pattern)) (get-branch-label branch))
        (list (get-branch-label branch)
              (caddr (longest-common-prefix (get-branch-label branch) pattern))
              (get-branch-subtree branch)
        ))
       (else (list #f (car (longest-common-prefix (get-branch-label branch) pattern)))))
     (list #f '())))
...
(define (st-has-pattern? st pattern)
  (define output (match-pattern-with-label st pattern))
  (cond
    ((list? output)
     (if (list? (car output))
