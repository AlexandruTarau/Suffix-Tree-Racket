...
(define (suffixes->st labeling-func suffixes alphabet)
  ...
  (filter
   ...
   (map
    (lambda (letter)
      (define filtered-suffixes
        (filter
         (lambda (suffix)
           (equal? letter (car suffix)))
         suffixes
         ))
      (define labeling-func-res
        (if (null? filtered-suffixes) '()
            (labeling-func filtered-suffixes)))
      (if (null? labeling-func-res) '()
          (append
           (list (car labeling-func-res))
           (if (null? (car (cdr labeling-func-res))) '()
...
(define text->st
  (lambda (text)
    (lambda (labeling-func)
      (define text-with-$ (append text '(#\$)))
      (define suffixes (get-suffixes text-with-$))
      (define alphabet (sort (remove-duplicates text-with-$) char<?))
