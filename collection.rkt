...
(define-syntax-rule (collection-car xs) (stream-first xs))

(define-syntax-rule (collection-cdr xs) (stream-rest xs))

(define-syntax-rule (null-collection) empty-stream)

(define-syntax-rule (collection-null? xs) (stream-empty? xs))

(define-syntax-rule (collection-map f xs) (stream-map f xs))

(define-syntax-rule (collection-filter f xs) (stream-filter f xs))

(define-syntax-rule (collection-length xs) (stream-length xs))

(define-syntax-rule (collection-cddr xs) (stream-rest (stream-rest xs)))

(define-syntax-rule (collection-cadr xs) (stream-first (stream-rest xs)))

(define-syntax-rule (collection-caddr xs) (stream-first (stream-rest (stream-rest xs))))

(define-syntax-rule (collection-append xs ...) (stream-append xs ...))
